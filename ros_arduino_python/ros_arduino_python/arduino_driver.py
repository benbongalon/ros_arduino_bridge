...
class Arduino:
    ...
    def connect2(self):
        time.sleep(1) # give the firmware time to wake up
        try:
            test = self.get_baud()
            ...
            print("Serial Exception:", sys.exc_info())
            print("Traceback follows:", traceback.print_exc(file=sys.stdout))
            print("Cannot get baudrate from Arduino!")

        try:
            self.PID_RATE = self.get_pid_rate()
            self.PID_INTERVAL = 1000 / self.PID_RATE
            print("Arduino is ready.")
        except SerialException:
            print("Serial Exception:", sys.exc_info())
            print("Traceback follows:", traceback.print_exc(file=sys.stdout))
            print("Cannot get PID update rate from Arduino!") 
    ...
    def get_pid_rate(self):
        ''' Get the current PID update rate.
        '''
